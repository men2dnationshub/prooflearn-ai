...
def apply_brand():
    st.markdown('<style>.stApp{color:#061A40}[data-testid="stSidebar"]{background:#F2F6FC}[data-testid="stMetric"]{border:1px solid #D8E4F3;border-radius:12px;padding:12px}h1,h2,h3{color:#061A40}</style>', unsafe_allow_html=True)

def render_sidebar_status():
    initialise_state()
    with st.sidebar:
        st.markdown(f"### {APP_NAME}"); st.caption(f"Version {APP_VERSION}"); st.divider()
